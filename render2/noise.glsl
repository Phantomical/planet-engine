...
double _mix_7(dvec3 v){
	return mix(_mul_2(v), _mul_5(v), _voronoi_6(v));
}
//
// Description : Array and textureless GLSL 2D/3D/4D simplex 
//               noise functions.
...
//               https://github.com/stegu/webgl-noise
// 
dvec3 _simplex_8_mod289(dvec3 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}
dvec4 _simplex_8_mod289(dvec4 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}
dvec4 _simplex_8_permute(dvec4 x) {
     return _simplex_8_mod289(((x*34.0)+1.0)*x);
}
dvec4 _simplex_8_taylorInvSqrt(dvec4 r)
{
  return 1.79284291400159 - 0.85373472095314 * r;
}
double _simplex_8(dvec3 v)
  { 
  const dvec2  C = dvec2(1.0/6.0, 1.0/3.0) ;
  const dvec4  D = dvec4(0.0, 0.5, 1.0, 2.0);
...
// Permutations
  i = _simplex_8_mod289(i); 
  dvec4 p = _simplex_8_permute( _simplex_8_permute( _simplex_8_permute( 
             i.z + dvec4(0.0, i1.z, i2.z, 1.0 ))
           + i.y + dvec4(0.0, i1.y, i2.y, 1.0 )) 
           + i.x + dvec4(0.0, i1.x, i2.x, 1.0 ));
...
//Normalise gradients
  dvec4 norm = _simplex_8_taylorInvSqrt(dvec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));
  p0 *= norm.x;
  p1 *= norm.y;
  p2 *= norm.z;
  p3 *= norm.w;
...
double _fbm_9(dvec3 v)
{
	double _9_total = 0.0;
	double _9_maxAmplitude = 0.0;
	double _9_amplitude = 1.0;
	double _9_frequency = 50.0;
	double _9_octaves = 16;
	double _9_persistance = 0.2;
	double _9_lacunarity = 2.0;
	for (uint _9_i = 0; _9_i < _9_octaves; ++_9_i)
	{
		_9_total += _simplex_8(v * _9_frequency) * _9_amplitude;
		_9_frequency *= _9_lacunarity;
		_9_maxAmplitude += _9_amplitude;
		_9_amplitude *= _9_persistance;
	}
	return _9_total / _9_maxAmplitude;
}
double _mul_10(dvec3 v){
	return (_fbm_9(v))*(50.0);
}
...
//               https://github.com/stegu/webgl-noise
// 
dvec3 _simplex_13_mod289(dvec3 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}
dvec4 _simplex_13_mod289(dvec4 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}
dvec4 _simplex_13_permute(dvec4 x) {
     return _simplex_13_mod289(((x*34.0)+1.0)*x);
}
dvec4 _simplex_13_taylorInvSqrt(dvec4 r)
{
  return 1.79284291400159 - 0.85373472095314 * r;
}
double _simplex_13(dvec3 v)
  { 
  const dvec2  C = dvec2(1.0/6.0, 1.0/3.0) ;
  const dvec4  D = dvec4(0.0, 0.5, 1.0, 2.0);
...
// Permutations
  i = _simplex_13_mod289(i); 
  dvec4 p = _simplex_13_permute( _simplex_13_permute( _simplex_13_permute( 
             i.z + dvec4(0.0, i1.z, i2.z, 1.0 ))
           + i.y + dvec4(0.0, i1.y, i2.y, 1.0 )) 
           + i.x + dvec4(0.0, i1.x, i2.x, 1.0 ));
...
//Normalise gradients
  dvec4 norm = _simplex_13_taylorInvSqrt(dvec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));
  p0 *= norm.x;
  p1 *= norm.y;
  p2 *= norm.z;
  p3 *= norm.w;
...
double _fbm_14(dvec3 v)
{
	double _14_total = 0.0;
	double _14_maxAmplitude = 0.0;
	double _14_amplitude = 1.0;
	double _14_frequency = 1.0;
	double _14_octaves = 8.0;
	double _14_persistance = 0.5;
	double _14_lacunarity = 2.0;
	for (uint _14_i = 0; _14_i < _14_octaves; ++_14_i)
	{
		_14_total += _simplex_13(v * _14_frequency) * _14_amplitude;
		_14_frequency *= _14_lacunarity;
		_14_maxAmplitude += _14_amplitude;
		_14_amplitude *= _14_persistance;
	}
	return _14_total / _14_maxAmplitude;
}
double _mul_15(dvec3 v){
	return (_fbm_14(v))*(1000.0);
}
double _add_16(dvec3 v){
	return (_sub_12(v))+(_mul_15(v));
}
double _max_17(dvec3 v){
	return max(_add_16(v), 0.0);
}
