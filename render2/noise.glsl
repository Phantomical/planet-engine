...
double _fbm_1(dvec3 v)
{
	...
	{
		_1_total += _simplex_0(v * _1_frequency) * _1_amplitude;
...
double _mul_2(dvec3 v){
	return (_fbm_1(v))*(150.0);
}
//
...
// https://github.com/stegu/webgl-noise
dvec3 _voronoi_3_mod289(dvec3 x) {
// Modulo 289 without a division (only multiplications)
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}
dvec3 _voronoi_3_mod7(dvec3 x) {
// Modulo 7 without a division
  return x - floor(x * (1.0 / 7.0)) * 7.0;
}
dvec3 _voronoi_3_permute(dvec3 x) {
// Permutation polynomial: (34x^2 + x) mod 289
  return _voronoi_3_mod289((34.0 * x + 1.0) * x);
}
dvec2 _voronoi_3_source(dvec3 P) {
...
#define jitter 1.0 // smaller jitter gives more regular pattern
	...
	dvec3 Pi = _voronoi_3_mod289(floor(P));
	...
	dvec3 p = _voronoi_3_permute(Pi.x + dvec3(-1.0, 0.0, 1.0));
	dvec3 p1 = _voronoi_3_permute(p + Pi.y - 1.0);
	dvec3 p2 = _voronoi_3_permute(p + Pi.y);
	dvec3 p3 = _voronoi_3_permute(p + Pi.y + 1.0);

	dvec3 p11 = _voronoi_3_permute(p1 + Pi.z - 1.0);
	dvec3 p12 = _voronoi_3_permute(p1 + Pi.z);
	dvec3 p13 = _voronoi_3_permute(p1 + Pi.z + 1.0);

	dvec3 p21 = _voronoi_3_permute(p2 + Pi.z - 1.0);
	dvec3 p22 = _voronoi_3_permute(p2 + Pi.z);
	dvec3 p23 = _voronoi_3_permute(p2 + Pi.z + 1.0);

	dvec3 p31 = _voronoi_3_permute(p3 + Pi.z - 1.0);
	dvec3 p32 = _voronoi_3_permute(p3 + Pi.z);
	dvec3 p33 = _voronoi_3_permute(p3 + Pi.z + 1.0);

	dvec3 ox11 = fract(p11*K) - Ko;
	dvec3 oy11 = _voronoi_3_mod7(floor(p11*K))*K - Ko;
	dvec3 oz11 = floor(p11*K2)*Kz - Kzo; // p11 < 289 guaranteed

	dvec3 ox12 = fract(p12*K) - Ko;
	dvec3 oy12 = _voronoi_3_mod7(floor(p12*K))*K - Ko;
	dvec3 oz12 = floor(p12*K2)*Kz - Kzo;

	dvec3 ox13 = fract(p13*K) - Ko;
	dvec3 oy13 = _voronoi_3_mod7(floor(p13*K))*K - Ko;
	dvec3 oz13 = floor(p13*K2)*Kz - Kzo;

	dvec3 ox21 = fract(p21*K) - Ko;
	dvec3 oy21 = _voronoi_3_mod7(floor(p21*K))*K - Ko;
	dvec3 oz21 = floor(p21*K2)*Kz - Kzo;

	dvec3 ox22 = fract(p22*K) - Ko;
	dvec3 oy22 = _voronoi_3_mod7(floor(p22*K))*K - Ko;
	dvec3 oz22 = floor(p22*K2)*Kz - Kzo;

	dvec3 ox23 = fract(p23*K) - Ko;
	dvec3 oy23 = _voronoi_3_mod7(floor(p23*K))*K - Ko;
	dvec3 oz23 = floor(p23*K2)*Kz - Kzo;

	dvec3 ox31 = fract(p31*K) - Ko;
	dvec3 oy31 = _voronoi_3_mod7(floor(p31*K))*K - Ko;
	dvec3 oz31 = floor(p31*K2)*Kz - Kzo;

	dvec3 ox32 = fract(p32*K) - Ko;
	dvec3 oy32 = _voronoi_3_mod7(floor(p32*K))*K - Ko;
	dvec3 oz32 = floor(p32*K2)*Kz - Kzo;

	dvec3 ox33 = fract(p33*K) - Ko;
	dvec3 oy33 = _voronoi_3_mod7(floor(p33*K))*K - Ko;
...
double _voronoi_3_selector(dvec2 v)
{
	return v.y - v.x;
}
double _voronoi_3(dvec3 v)
{
    return _voronoi_3_selector(_voronoi_3_source(v));
}
double _fbm_4(dvec3 v)
{
	...
	{
		_4_total += _voronoi_3(v * _4_frequency) * _4_amplitude;
...
double _mul_5(dvec3 v){
	return (_fbm_4(v))*(50.0);
}
//
// Description : Array and textureless GLSL 2D/3D/4D simplex 
//               noise functions.
...
//               https://github.com/stegu/webgl-noise
// 
dvec3 _simplex_6_mod289(dvec3 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}
dvec4 _simplex_6_mod289(dvec4 x) {
  return x - floor(x * (1.0 / 289.0)) * 289.0;
}
dvec4 _simplex_6_permute(dvec4 x) {
     return _simplex_6_mod289(((x*34.0)+1.0)*x);
}
dvec4 _simplex_6_taylorInvSqrt(dvec4 r)
{
  return 1.79284291400159 - 0.85373472095314 * r;
}
double _simplex_6(dvec3 v)
  { 
  const dvec2  C = dvec2(1.0/6.0, 1.0/3.0) ;
  const dvec4  D = dvec4(0.0, 0.5, 1.0, 2.0);
...
// Permutations
  i = _simplex_6_mod289(i); 
  dvec4 p = _simplex_6_permute( _simplex_6_permute( _simplex_6_permute( 
             i.z + dvec4(0.0, i1.z, i2.z, 1.0 ))
           + i.y + dvec4(0.0, i1.y, i2.y, 1.0 )) 
           + i.x + dvec4(0.0, i1.x, i2.x, 1.0 ));
...
//Normalise gradients
  dvec4 norm = _simplex_6_taylorInvSqrt(dvec4(dot(p0,p0), dot(p1,p1), dot(p2, p2), dot(p3,p3)));
  p0 *= norm.x;
  p1 *= norm.y;
  p2 *= norm.z;
  p3 *= norm.w;
...
double _fbm_7(dvec3 v)
{
	double _7_total = 0.0;
	double _7_maxAmplitude = 0.0;
	double _7_amplitude = 1.0;
	double _7_frequency = 50.0;
	double _7_octaves = 3;
	double _7_persistance = 0.5;
	double _7_lacunarity = 2.0;
	for (uint _7_i = 0; _7_i < _7_octaves; ++_7_i)
	{
		_7_total += _simplex_6(v * _7_frequency) * _7_amplitude;
		_7_frequency *= _7_lacunarity;
		_7_maxAmplitude += _7_amplitude;
		_7_amplitude *= _7_persistance;
	}
	return _7_total / _7_maxAmplitude;
}
double _mix_8(dvec3 v){
	return mix(_mul_2(v), _mul_5(v), _fbm_7(v));
}
//
// Description : Array and textureless GLSL 2D/3D/4D simplex 
//               noise functions.
...
double _fbm_10(dvec3 v)
{
	...
	double _10_frequency = 1.0;
	double _10_octaves = 8;
...
double _mul_11(dvec3 v){
	return (_fbm_10(v))*(1000.0);
}
